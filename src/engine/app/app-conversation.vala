...
public class Geary.App.Conversation : BaseObject {
     ...
     */
    public int get_count(bool folder_email_ids_only = false) {
    ...
    private Geary.Account get_account() {
        if (owner == null)
            error("Can't manipulate conversation %s after its owner has been cleared", to_string());
        return owner.folder.account;
    }
    ...
    private async Gee.HashMap<Geary.FolderPath, Geary.Folder> get_folder_instances_async(
        ...
        foreach (Geary.FolderPath path in paths) {
            Geary.Folder folder = yield get_account().fetch_folder_async(path, cancellable);
    ...
    // TODO: how to support multiple selected conversations at once?
    public async void mark_async(Geary.EmailFlags? flags_to_add, Geary.EmailFlags? flags_to_remove,
        Gee.Collection<Geary.EmailIdentifier>? emails = null, Cancellable? cancellable = null) throws Error {
        Gee.MultiMap<Geary.EmailIdentifier, Geary.FolderPath>? ids_to_folders
            = yield get_account().get_containing_folders_async(emails ?? this.emails.keys, cancellable);
        Gee.MultiMap<Geary.FolderPath, Geary.EmailIdentifier> folders_to_ids
